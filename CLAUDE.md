...
`garmin_connections` (added in `006_add_garmin_connection.sql`): one row per user holding the **encrypted** Garmin Connect session token (OAuth1+OAuth2). The password is never stored — `POST /api/garmin/connect` logs in once via the `garmin-connect` npm lib, then persists `exportToken()` output AES-256-GCM-encrypted (`lib/garmin/crypto.ts`; key derived from `GARMIN_TOKEN_KEY` or falls back to `SUPABASE_SERVICE_ROLE_KEY`). RLS scopes rows to the owner. Garmin import flow: Settings → Garmin Connect (`components/journal/GarminConnect.tsx`) links the account; New Entry → "Import from Garmin" (`GarminActivityPicker` + `step==='garmin'` in `NewTripForm`) lists fishing activities (`/api/garmin/activities`), and picking one calls `/api/garmin/import` → downloads the original FIT → `lib/garmin/fit.ts` reads each **manually-triggered lap** (`lapTrigger==='manual'`) as one catch (that's how the watch's "Log Catch" is stored) → pre-fills step 2 with the trip location/date and one pinned catch per catch (lat/lng → fish map markers). Garmin routes are `runtime='nodejs'` (the lib uses Node fs/https) and unofficial (can break if Garmin changes their SSO); login is from a datacenter IP so Garmin's bot-protection may occasionally block it.
